...
{$where}
ORDER BY 
    CASE WHEN s.is_main_store = 1 THEN 0 ELSE 1 END,
    b.branch_name,
    FIELD(s.store_category, 'main_store', 'warehouse', 'cold', 'narcotics', 'cosmetics', 'medical_supply', 'surplus', 'damaged', 'expired'),
    s.store_name";
...
$total_main = count(array_filter($all_stores, fn($s) => $s['is_main_store']));
$total_sub = $total_stores - $total_main;
$total_products = array_sum(array_column($all_stores, 'products_count'));
$total_stock_value = array_sum(array_column($all_stores, 'total_value'));
...
<head>
    ...
    <style>
        ...
        .branch-header.main-branch { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
        ...
        .store-card.sub-store { border-right-color: #764ba2; }
        ...
        .store-card .cat-badge { font-size: 11px; padding: 4px 10px; border-radius: 8px; }
        .store-card .main-badge { 
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
            color: white; 
            font-size: 10px; 
            padding: 3px 8px; 
        ...
        <div class="container-fluid">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <div class="stat-card">
                        ...
                        <div class="value"><?= number_format($total_stores) ?></div>
                ...
                <div class="col-md-3">
                    <div class="stat-card">
                        ...
                        <div class="value"><?= number_format($total_main) ?></div>
                        <div class="label">مخازن رئيسية</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="stat-card">
                        <i class="bi bi-boxes" style="color: var(--secondary)"></i>
                        <div class="value"><?= number_format($total_sub) ?></div>
                        <div class="label">مخازن فرعية</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="stat-card">
                        <i class="bi bi-boxes" style="color: #198754"></i>
            ...
            <?php else: ?>
                ...
                <div class="branch-section">
                    <div class="branch-header <?= $branchKey === 0 ? 'main-branch' : '' ?>">
                        <div>
                            <i class="bi bi-building"></i>
                            <h4 class="d-inline-block me-2"><?= htmlspecialchars($group['branch_name']) ?></h4>
                            <?php if ($group['branch_code']): ?>
                                <span class="badge"><?= htmlspecialchars($group['branch_code']) ?></span>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <?php
                            $cat_info = $category_labels[$store['store_category']] ?? ['مخزن', 'bi-box', 'secondary', 'border-secondary'];
                            $is_main = $store['is_main_store'];
                            $card_class = $is_main ? 'main-store' : 'sub-store';
                                ...
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div>
                                            ...
                                            <?php if ($is_main): ?>
                                                <span class="main-badge ms-1"><i class="bi bi-star-fill"></i> رئيسي</span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="btn-group">
                                            <a href="view.php?id=<?= $store['id'] ?>" class="btn btn-sm btn-outline-info" title="عرض">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="edit.php?id=<?= $store['id'] ?>" class="btn btn-sm btn-outline-warning" title="تعديل">
                                    ...
                                    <span class="badge bg-<?= $cat_info[2] ?> cat-badge">
                                        <i class="bi <?= $cat_info[1] ?>"></i> <?= $cat_info[0] ?>
                                    </span>

                                    <div class="stats-row">
                                        <div class="stat">
                                            <div class="val"><?= number_format($products_count) ?></div>
                                            <div class="lbl">صنف</div>
                                        </div>
                                        <div class="stat">
                                            <div class="val"><?= number_format($total_stock, 0) ?></div>
                                            <div class="lbl">كمية</div>
                                        </div>
                                        <div class="stat">
                                            <div class="val"><?= number_format($total_value, 0) ?></div>
                                            <div class="lbl">قيمة</div>
                                        </div>
                                    </div>

                                    <?php if (!empty($store['notes'])): ?>
                                        <div class="mt-2 text-muted small">
                                            <i class="bi bi-info-circle"></i> <?= htmlspecialchars(mb_substr($store['notes'], 0, 60)) ?><?= mb_strlen($store['notes']) > 60 ? '...' : '' ?>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
