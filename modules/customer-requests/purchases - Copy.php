...
// Handle mark as purchased (على سعر مورد معين)
...
// Handle add new supplier from modal
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_supplier_price') {
    ...
    try {
        $stmt = $db->prepare("SELECT product_id, product_code FROM order_items WHERE id = ?");
        $stmt->execute([$item_id]);
        $item = $stmt->fetch();
        ...
        // Delete old price for same product + supplier
        $stmt = $db->prepare("DELETE FROM supplier_prices WHERE product_id = ? AND supplier_id = ?");
        $stmt->execute([$item['product_id'], $supplier_id]);
        
        // Insert new price
        $stmt = $db->prepare("INSERT INTO supplier_prices 
            (product_id, product_code, supplier_id, supplier_price, sell_price, profit_margin, notes, delivery_time_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $item['product_id'], $item['product_code'], $supplier_id, 
            $supplier_price, $sell_price, $profit_margin, $notes, $delivery_time_id
        ]);
        
        echo json_encode(['success' => true, 'message' => 'تم إضافة السعر بنجاح']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
    exit;
...
foreach ($allPrices as $price) {
    $supplierPrices[$price['product_id']][] = $price;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            ...
            <?php if (count($need_purchase_items) > 0): ?>
                <?php foreach ($need_purchase_items as $item): 
                    $prices = $supplierPrices[$item['product_id']] ?? $supplierPrices[$item['product_code']] ?? [];
                ?>
                <div class="item-card">
                    <div class="row">
                        <div class="col-md-3">
                            <h6 class="mb-1"><?= $item['product_name'] ?></h6>
                            <?php if ($item['product_code']): ?><small class="text-muted">كود: <?= $item['product_code'] ?></small><?php endif; ?>
                            <div class="mt-2">
                        ...
                        <div class="col-md-3 text-end">
                            <button class="btn btn-info btn-action" onclick="showPriceModal(<?= $item['id'] ?>, '<?= $item['product_name'] ?>', <?= $item['quantity'] ?>)">
                                <i class="bi bi-tag"></i> إضافة/اختيار سعر
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="supplier-price" id="price-<?= $price['supplier_id'] ?>-<?= $item['id'] ?>">
                            <div class="row">
                                <div class="col-md-4">
                                    <strong><?= $price['supplier_name'] ?></strong>
                                    <small class="text-muted d-block"><?= $price['supplier_code'] ?></small>
                                ...
                                <div class="col-md-2 text-end">
                                    <button class="btn btn-success btn-sm w-100 mb-1" onclick="purchaseFromSupplier(<?= $item['id'] ?>, <?= $price['supplier_id'] ?>, <?= $price['supplier_price'] ?>, <?= $price['sell_price'] ?>, <?= $price['profit_margin'] ?>, <?= $item['quantity'] ?>)">
                                        <i class="bi bi-check-lg"></i> شراء من هنا
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="alert alert-warning mt-2">
                        <i class="bi bi-exclamation-triangle"></i> لا يوجد أسعار موردين مسجلة. اضغط "إضافة/اختيار سعر" لإضافة سعر.
                    </div>
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
        ...
        <div class="content-card">
            ...
            <div class="purchased-item">
                <div class="row">
                    <div class="col-md-3">
                        <strong><?= $ph['product_name'] ?></strong>
                        <small class="text-muted d-block">كود: <?= $ph['product_code'] ?></small>
                    </div>
                    <div class="col-md-3">
                        <div><strong>المورد:</strong> <?= $ph['supplier_name'] ?></div>
                        <small class="text-muted"><?= $ph['supplier_code'] ?> | <?= $ph['phone'] ?: 'لا يوجد تليفون' ?></small>
                    </div>
                    <div class="col-md-3">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">إضافة/اختيار سعر مورد</h5>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="modalDeliveryTime">
                                    ...
                                    <option value="<?= $dt['id'] ?>"><?= $dt['time_name'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        ...
        function showPriceModal(itemId, productName, quantity) {
