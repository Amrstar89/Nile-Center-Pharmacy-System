 ...
 * ============================================
 * تُستخدم في: التحويلات، الجرد، الرصيد الافتتاحي، تعديل الأسعار
 * 
 * Parameters (GET):
 ...
 * 
 * مكونات الشاشة:
 *   1. فلترات بحث متعددة (اسم، باركود، كود، مادة فعالة، شركة، تصنيف)
 *   2. فلتر نطاق سعر (من - إلى)
 *   3. checkbox "بدون رصيد"
 *   4. جدول نتائج مع ترقيم صفحات
 *   5. عند اختيار صنف: تواريخ الصلاحية + كميات + أسعار
 *   6. أزرار: موافق / إلغاء
 */
require_once __DIR__ . '/../core/config.php';

// This file can be called directly or included
// If called directly, show the popup UI
if (basename($_SERVER['PHP_SELF']) === 'product-search-popup-new.php') {
    ...
    // Get active ingredients for filter
    $ingredients = $db->query("SELECT DISTINCT scientific_name FROM products WHERE scientific_name IS NOT NULL AND scientific_name != '' ORDER BY scientific_name LIMIT 100")->fetchAll();
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .results-header h6 { margin: 0; color: #555; }
        ...
        .results-table tbody tr.no-stock {
            opacity: 0.5;
        }
        ...
        .detail-panel .product-title {
            font-size: 16px;
            font-weight: 700;
            color: var(--primary);
            margin-bottom: 10px;
        }
        .detail-panel .product-info {
            display: flex;
            gap: 20px;
            margin-bottom: 12px;
            flex-wrap: wrap;
        ...
        .detail-panel .info-item .label { font-size: 10px; color: #888; }
        .detail-panel .info-item .value { font-size: 14px; font-weight: 700; color: #333; }
        ...
        .batch-table tr:hover { background: #f0f4ff; }
        .exp-badge {
            padding: 2px 8px;
            border-radius: 6px;
            font-size: 10px;
        ...
        .qty-section input, .qty-section select {
            font-size: 14px;
            font-weight: 600;
            text-align: center;
        }
        .qty-section .total-display {
            background: linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%);
            color: white;
            padding: 8px 20px;
            border-radius: 8px;
        ...
        .item-count-badge .count { font-weight: 700; color: var(--primary); }
...
<body>
    ...
    <div class="filter-panel">
        ...
        <div class="input-group mb-2">
            <input type="text" id="searchInput" class="form-control search-main-input" 
                   placeholder="اكتب هنا للبحث..." onkeyup="handleSearchKey(event)" autocomplete="off">
        ...
        <div class="text-center">
            <button class="btn btn-sm btn-outline-secondary" type="button" onclick="toggleAdvanced()">
        ...
        <div class="advanced-filters d-none" id="advancedFilters">
            <div class="filter-row">
                ...
                <div class="filter-group" style="flex: 0.5;">
                    ...
                    <input type="number" id="priceFrom" class="form-control" placeholder="0" onchange="doSearch()">
                </div>
                <div class="filter-group" style="flex: 0.5;">
                    <label>السعر إلى</label>
                    <input type="number" id="priceTo" class="form-control" placeholder="∞" onchange="doSearch()">
                </div>
                <div class="filter-group" style="flex: 0 0 auto;">
                    <div class="form-check form-switch mt-4">
                        <input class="form-check-input" type="checkbox" id="showNoStock" onchange="doSearch()">
                        <label class="form-check-label" for="showNoStock" style="font-size: 11px;">بدون رصيد</label>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="results-container">
        <div class="results-header">
            <h6><i class="bi bi-list-ul"></i> نتائج البحث</h6>
            <span class="badge bg-primary" id="resultCount">0 صنف</span>
        </div>
        <div class="scrollable-table" id="resultsTableContainer">
    ...
    <div class="detail-panel hidden" id="detailPanel">
        <div class="product-title" id="detailProductName">
            <i class="bi bi-check-circle-fill text-success"></i> <span></span>
        </div>
        <div class="product-info">
            <div class="info-item">
                <div class="label">كود الصنف</div>
                <div class="value" id="detailProductCode">-</div>
            </div>
            <div class="info-item">
                <div class="label">الباركود</div>
                <div class="value" id="detailBarcode">-</div>
            </div>
            <div class="info-item">
                <div class="label">الشركة</div>
                <div class="value" id="detailCompany">-</div>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
                ...
                <div class="value" id="detailTotalStock" style="color: var(--success);">-</div>
            </div>
        </div>
        
        <!-- Batches Table (for expiry products) -->
        ...
        <div class="qty-section">
            <div class="form-group">
                <label>الوحدة</label>
                <input type="text" id="detailUnit" class="form-control" value="علبة" readonly style="width: 80px;">
            </div>
            <div class="form-group">
                <label>الكمية</label>
                <input type="number" id="detailQty" class="form-control" value="1" min="0.001" step="0.001" 
                       style="width: 100px;" onchange="calcDetailTotal()">
            </div>
            <div class="form-group">
                <label>تاريخ الصلاحية</label>
                <select id="detailExpDate" class="form-select" style="width: 150px;" onchange="onExpDateChange()">
                    <option value="">أقرب تاريخ</option>
                </select>
            </div>
            <div class="form-group">
                <label>سعر البيع</label>
                <input type="number" id="detailSellPrice" class="form-control" step="0.01" min="0" 
                       style="width: 100px;" onchange="calcDetailTotal()">
            </div>
            <div class="form-group">
                <label>تكلفة الوحدة</label>
                <input type="number" id="detailCost" class="form-control" step="0.01" min="0" 
                       style="width: 100px;" readonly>
            </div>
            <div class="total-display" id="detailTotal">33.00</div>
        </div>
    </div>
    ...
    <div class="popup-footer">
        <div class="item-count-badge">
            <i class="bi bi-boxes"></i> عدد الأصناف: <span class="count" id="itemCount">0</span>
        </div>
        <div>
            <button class="btn btn-secondary" onclick="window.close()">
                <i class="bi bi-x-lg"></i> إلغاء
            </button>
            <button class="btn btn-success btn-lg" onclick="confirmSelection()" id="btnConfirm" disabled>
                <i class="bi bi-check-lg"></i> موافق
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        function setSearchType(type) {
            ...
            const placeholders = {
                name: 'اكتب اسم الصنف (عربي أو إنجليزي)...',
                barcode: 'ادخل الباركود...',
                code: 'ادخل كود الصنف...',
                scientific: 'اكتب المادة الفعالة...',
                company: 'ابحث باسم الشركة...'
            };
            document.getElementById('searchInput').placeholder = placeholders[type] || 'اكتب هنا للبحث...';
        ...
        async function doSearch() {
            ...
            // If query is empty and no filters, show message
        ...
        function renderResults(products) {
            ...
            tbody.innerHTML = products.map((p, i) => {
                ...
                return `
                <tr class="${noStockClass}" onclick="selectProduct(${i})" id="row_${i}">
                    <td><span class="product-code">${p.manual_code || p.product_code || '-'}</span></td>
                    <td>
                        <span class="product-name">${p.product_name}</span>
                        ${p.barcode ? `<br><small class="text-muted">${p.barcode}</small>` : ''}
                    </td>
                    <td class="sell-price">${parseFloat(p.sell_price).toFixed(2)}</td>
                    <td><small class="text-muted">${p.scientific_name || '-'}</small></td>
                    <td><span class="stock-qty ${stockClass}">${parseFloat(p.stock_qty).toFixed(0)}</span></td>
                </tr>`;
            }).join('');
        }
        ...
        function selectProduct(index) {
            ...
            // Fill product info
            document.querySelector('#detailProductName span').textContent = p.product_name;
            document.getElementById('detailProductCode').textContent = p.manual_code || p.product_code || '-';
            ...
            document.getElementById('detailTotalStock').textContent = parseFloat(p.stock_qty).toFixed(2);
            document.getElementById('detailSellPrice').value = parseFloat(p.sell_price).toFixed(2);
            document.getElementById('detailCost').value = parseFloat(p.unit_cost || p.cost_price).toFixed(2);
            document.getElementById('detailUnit').value = p.unit_name_ar || 'علبة';
            
            // Fill batches/expiry dates
            const expSelect = document.getElementById('detailExpDate');
            expSelect.innerHTML = '<option value="">أقرب تاريخ</option>';
            
            if (p.batches && p.batches.length > 0) {
                document.getElementById('batchSection').style.display = 'block';
                document.getElementById('batchBody').innerHTML = p.batches.map(b => {
                    ...
                    return `<tr>
                        <td><span class="exp-badge ${expClass}">${b.exp_date}</span></td>
                        <td>${parseFloat(b.remaining_qty).toFixed(2)}</td>
                        <td>${parseFloat(b.unit_cost).toFixed(2)}</td>
                    </tr>`;
                }).join('');
        ...
        function onExpDateChange() {
            const select = document.getElementById('detailExpDate');
            const opt = select.options[select.selectedIndex];
            if (opt && opt.dataset.qty) {
                // You can auto-adjust max qty here if needed
            }
        }
        ...
        function showError(msg) {
            document.getElementById('resultsBody').innerHTML = 
                `<tr><td colspan="5" class="text-center py-4 text-danger">
                    <i class="bi bi-exclamation-triangle" style="font-size: 24px;"></i><br>${msg}
                </td></tr>`;
        }
        ...
        function confirmSelection() {
            ...
            const result = {
                ...
                has_expire: selectedProduct.has_expire || false
            };
            ...
            if (window.opener && window.opener[CALLBACK]) {
                ...
                window.parent.postMessage({ type: 'product-selected', callback: CALLBACK, data: result }, '*');
            }
        }
        
        // Listen for messages from parent (for iframe mode)
        window.addEventListener('message', (e) => {
            if (e.data.type === 'set-store') {
