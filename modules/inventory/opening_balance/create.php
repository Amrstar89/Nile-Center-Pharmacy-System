        ...
        foreach ($_POST['items'] as $item) {
            if (!empty($item['product_id']) && !empty($item['quantity']) && $item['quantity'] > 0) {
                ...
                $db->prepare("
                    ...
                    (transaction_type, reference_type, store_id, product_id, quantity, quantity_base, unit_cost, unit_price, discount_percent, vat_percent, total_cost, notes, created_by, created_at)
                ...
                if ($existing->fetch()) {
                    $db->prepare("
                        ...
                        WHERE store_id = ? AND product_id = ? AND batch_id IS NULL
                    ")->execute([$quantity, $unit_cost, $sell_price, $discount, $vat, $store_id, $product_id]);
                } else {
                    $db->prepare("
                        INSERT INTO inventory_items 
                        (store_id, product_id, quantity, unit_cost, sell_price, discount_percent, vat_percent, reorder_point, max_stock, is_active, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 1, NOW())
...
<body>
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        ...
                                        <div class="input-group">
                                            <input type="text" id="product_name_0" class="form-control" readonly placeholder="اضغط F2 للبحث">
    ...
    <?php include __DIR__ . '/../../../includes/product-search-popup.php'; ?>
    ...
    <script>
        ...
        function fillProductData(index, product) {
            ...
            document.getElementById('product_name_' + index).value = product.product_name;
            ...
            newRow.innerHTML = `
                ...
                <div class="row">
                    <div class="col-md-3">
                        <label class="form-label">الصنف <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <input type="text" id="product_name_${itemIndex}" class="form-control" readonly placeholder="اضغط F2 للبحث">
