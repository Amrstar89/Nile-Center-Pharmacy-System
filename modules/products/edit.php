...
$product = $db->prepare("SELECT * FROM products WHERE id = ? AND is_active = 1");
...
function editOld($field, $productValue) {
    return isset($_POST[$field]) ? htmlspecialchars($_POST[$field]) : htmlspecialchars($productValue ?? '');
}

function editOldCheck($field, $productValue) {
    return isset($_POST[$field]) ? 'checked' : ($productValue ? 'checked' : '');
}

function editOldSelect($field, $value, $productValue) {
    return isset($_POST[$field]) ? ($_POST[$field] == $value ? 'selected' : '') : ($productValue == $value ? 'selected' : '');
}
...
$locations = $db->prepare("SELECT * FROM product_locations WHERE product_id = ?");
$locations->execute([$product_id]);
$locations = $locations->fetchAll();

$alerts = $db->prepare("SELECT alert_type FROM product_alerts WHERE product_id = ?");
$alerts->execute([$product_id]);
$existing_alerts = $alerts->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($product_name)) {
            ...
            $stmt = $db->prepare("SELECT id FROM products WHERE product_name = ? AND is_active = 1 AND id != ?");
        ...
        if (!empty($product_name_en)) {
            $stmt = $db->prepare("SELECT id FROM products WHERE product_name_en = ? AND is_active = 1 AND id != ?");
            $stmt->execute([$product_name_en, $product_id]);
            if ($stmt->fetch()) {
        ...
        if (!empty($_POST['barcodes'])) {
            foreach ($_POST['barcodes'] as $barcode) {
                if (!empty($barcode)) {
                    $stmt = $db->prepare("SELECT pb.id FROM product_barcodes pb JOIN products p ON pb.product_id = p.id WHERE pb.barcode = ? AND p.is_active = 1 AND pb.product_id != ?");
        ...
        if (!empty($qr_code)) {
            $stmt = $db->prepare("SELECT pb.id FROM product_barcodes pb JOIN products p ON pb.product_id = p.id WHERE pb.barcode = ? AND p.is_active = 1 AND pb.product_id != ?");
            $stmt->execute([$qr_code, $product_id]);
            if ($stmt->fetch()) {
        ...
        if (!empty($_POST['barcodes'])) {
            ...
            foreach ($_POST['barcodes'] as $index => $barcode) {
                if (!empty($barcode)) {
                    $barcode_stmt->execute([$product_id, $barcode, $_POST['barcode_units'][$index] ?? 1, $index === 0 ? 1 : 0]);
                }
            }
        }
        ...
        if (!empty($qr_code)) {
            ...
            $qr_stmt->execute([$product_id, $qr_code, 1, 0]);
        }

        $db->prepare("DELETE FROM product_locations WHERE product_id = ?")->execute([$product_id]);
        if (!empty($_POST['locations'])) {
            ...
            foreach ($_POST['locations'] as $index => $location) {
                if (!empty($location)) {
                    $location_stmt->execute([$product_id, $location, $_POST['location_names'][$index] ?? $location, $_POST['shelf_numbers'][$index] ?? null, $_POST['row_numbers'][$index] ?? null]);
                }
            }
        }

        $db->prepare("DELETE FROM product_alerts WHERE product_id = ?")->execute([$product_id]);
        if (!empty($_POST['alerts'])) {
            ...
            foreach ($_POST['alerts'] as $alert_type) {
                if (!empty($alert_type)) {
                    $alert_stmt->execute([$product_id, $alert_type, getAlertMessage($alert_type)]);
                }
            }
        }
        ...
        if (!empty($_POST['custom_alert'])) {
            ...
            $custom_stmt->execute([$product_id, 'OTHER', $_POST['custom_alert']]);
        }
    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="index.php" class="btn btn-secondary"><i class="bi bi-arrow-right"></i> العودة للقائمة</a>
            </div>
                ...
                <div class="card">
                    <div class="card-header p-0">
                        <ul class="nav nav-tabs" id="productTabs" role="tablist">
                            ...
                            <li class="nav-item"><a class="nav-link" id="barcodes-tab" data-bs-toggle="tab" href="#barcodes" role="tab"><i class="bi bi-upc"></i> الباركود</a></li>
                            <li class="nav-item"><a class="nav-link" id="pricing-tab" data-bs-toggle="tab" href="#pricing" role="tab"><i class="bi bi-tag"></i> التسعير والوحدات</a></li>
                            <li class="nav-item"><a class="nav-link" id="stock-tab" data-bs-toggle="tab" href="#stock" role="tab"><i class="bi bi-warehouse"></i> المخزون</a></li>
                            <li class="nav-item"><a class="nav-link" id="alerts-tab" data-bs-toggle="tab" href="#alerts" role="tab"><i class="bi bi-exclamation-triangle"></i> التحذيرات</a></li>
                        </ul>
                    </div>
                        ...
                        <div class="tab-content" id="productTabContent">
                            ...
                            <div class="tab-pane fade show active" id="basic" role="tabpanel">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="product_name" id="product_name" class="form-control" required 
                                                   value="<?= editOld('product_name', $product['product_name']) ?>" placeholder="اسم الصنف بالعربي">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="product_name_en" id="product_name_en" class="form-control" 
                                                   value="<?= editOld('product_name_en', $product['product_name_en']) ?>" placeholder="Product Name in English">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="scientific_name" class="form-control" 
                                                   value="<?= editOld('scientific_name', $product['scientific_name']) ?>" placeholder="Active Ingredient">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select name="product_type_id" class="form-select" required>
                                                <?php foreach ($product_types as $type): ?>
                                                    <option value="<?= $type['id'] ?>" <?= editOldSelect('product_type_id', $type['id'], $product['product_type_id']) ?>><?= $type['type_name_ar'] ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="category_id" class="form-select">
                                                ...
                                                <?php foreach ($categories as $cat): ?>
                                                    <option value="<?= $cat['id'] ?>" <?= editOldSelect('category_id', $cat['id'], $product['category_id']) ?>><?= $cat['category_name_ar'] ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="company_id" class="form-select">
                                                ...
                                                <?php foreach ($companies as $comp): ?>
                                                    <option value="<?= $comp['id'] ?>" <?= editOldSelect('company_id', $comp['id'], $product['company_id']) ?>><?= $comp['company_name_ar'] ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <select name="is_imported" class="form-select" required>
                                                <option value="0" <?= editOldSelect('is_imported', '0', $product['is_imported']) ?>>محلي</option>
                                                <option value="1" <?= editOldSelect('is_imported', '1', $product['is_imported']) ?>>مستورد</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" name="is_service" value="1" class="form-check-input" id="is_service" <?= editOldCheck('is_service', $product['is_service']) ?>>
                                            <label class="form-check-label" for="is_service">صنف خدمي</label>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" name="hide_in_receipt" value="1" class="form-check-input" id="hide_in_receipt" <?= editOldCheck('hide_in_receipt', $product['hide_in_receipt']) ?>>
                                            <label class="form-check-label" for="hide_in_receipt">إخفاء في الريسيت</label>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" name="is_shortage" value="1" class="form-check-input" id="is_shortage" <?= editOldCheck('is_shortage', $product['is_shortage']) ?>>
                                            <label class="form-check-label" for="is_shortage">نواقص عامة</label>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" name="has_expire" value="1" class="form-check-input" id="has_expire" <?= editOldCheck('has_expire', $product['has_expire']) ?>>
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" name="can_be_negative" value="1" class="form-check-input" id="can_be_negative" <?= editOldCheck('can_be_negative', $product['can_be_negative']) ?>>
                                            ...
                                            <div class="barcode-row row mb-2">
                                                <div class="col-md-8">
                                                    <input type="text" name="barcodes[]" class="form-control barcode-input" 
                                                           value="<?= htmlspecialchars($barcode['barcode']) ?>" placeholder="باركود">
                                                </div>
                                                <div class="col-md-4">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="mb-3">
                                            <input type="text" name="qr_code" id="qr_code" class="form-control" 
                                                   value="<?= editOld('qr_code', '') ?>" placeholder="QR Code">
                                            <div class="validation-feedback" id="qr_code_feedback"></div>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" name="print_barcode" value="1" class="form-check-input" id="print_barcode" <?= editOldCheck('print_barcode', $product['print_barcode']) ?>>
                                            <label class="form-check-label" for="print_barcode">طباعة باركود داخلي</label>
                                        </div>
                                        <div class="form-check mt-2">
                                            <input type="checkbox" name="print_internal_barcode" value="1" class="form-check-input" id="print_internal_barcode" <?= editOldCheck('print_internal_barcode', $product['print_internal_barcode']) ?>>
                                ...
                                <div class="row mb-4">
                                    <div class="col-md-3">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="sell_price" id="sell_price" class="form-control" step="0.01" 
                                                   value="<?= editOld('sell_price', $product['sell_price']) ?>" placeholder="0.00" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="profit_margin" id="profit_margin" class="form-control" step="0.01" 
                                                       value="<?= editOld('profit_margin', $product['profit_margin'] ?? 0) ?>" placeholder="0.00">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="cost_price" id="cost_price" class="form-control" step="0.01" 
                                                       value="<?= editOld('cost_price', $product['cost_price']) ?>" placeholder="0.00">
                                    ...
                                    <div class="col-md-3">
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="vat_percent" id="vat_percent" class="form-control" step="0.01" 
                                                       value="<?= editOld('vat_percent', $product['vat_percent'] ?? 0) ?>">
                                                <span class="input-group-text">%</span>
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="unit1_id" class="form-select">
                                                    ...
                                                    <?php foreach ($units as $unit): ?>
                                                        <option value="<?= $unit['id'] ?>" <?= editOldSelect('unit1_id', $unit['id'], $product['unit1_id']) ?>><?= $unit['unit_name_ar'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                        ...
                                        <div class="unit-card">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="unit2_id" class="form-select">
                                                    <option value="">-- اختر --</option>
                                                    <?php foreach ($units as $unit): ?>
                                                        <option value="<?= $unit['id'] ?>" <?= editOldSelect('unit2_id', $unit['id'], $product['unit2_id']) ?>><?= $unit['unit_name_ar'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">عدد الوحدات</label>
                                                <input type="number" name="unit1_to_unit2" id="unit1_to_unit2" class="form-control" 
                                                       value="<?= editOld('unit1_to_unit2', $product['unit1_to_unit2'] ?? 1) ?>" placeholder="1">
                                                <small class="text-muted">الوحدة الكبرى = كم وحدة وسطى؟ (افتراضي: 1)</small>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">سعر البيع</label>
                                                <input type="number" name="unit2_sell_price" id="unit2_sell_price" class="form-control" step="0.01" 
                                                       value="<?= editOld('unit2_sell_price', $product['unit2_sell_price']) ?>" placeholder="0.00">
                                        ...
                                        <div class="unit-card">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="unit3_id" class="form-select">
                                                    <option value="">-- اختر --</option>
                                                    <?php foreach ($units as $unit): ?>
                                                        <option value="<?= $unit['id'] ?>" <?= editOldSelect('unit3_id', $unit['id'], $product['unit3_id']) ?>><?= $unit['unit_name_ar'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">عدد الوحدات</label>
                                                <input type="number" name="unit1_to_unit3" id="unit1_to_unit3" class="form-control" 
                                                       value="<?= editOld('unit1_to_unit3', $product['unit1_to_unit3'] ?? 1) ?>" placeholder="1">
                                                <small class="text-muted">الوحدة الكبرى = كم وحدة صغرى؟ (افتراضي: 1)</small>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">سعر البيع</label>
                                                <input type="number" name="unit3_sell_price" id="unit3_sell_price" class="form-control" step="0.01" 
                                                       value="<?= editOld('unit3_sell_price', $product['unit3_sell_price']) ?>" placeholder="0.00">
                                                <small class="price-sync">يتم حسابه تلقائي</small>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select name="default_sale_unit" class="form-select">
                                                <option value="1" <?= editOldSelect('default_sale_unit', '1', $product['default_sale_unit']) ?>>الوحدة الكبرى</option>
                                                <option value="2" <?= editOldSelect('default_sale_unit', '2', $product['default_sale_unit']) ?>>الوحدة الوسطى</option>
                                                <option value="3" <?= editOldSelect('default_sale_unit', '3', $product['default_sale_unit']) ?>>الوحدة الصغرى</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-check mb-3">
                                            <input type="checkbox" name="allow_discount" value="1" class="form-check-input" id="allow_discount" <?= editOldCheck('allow_discount', $product['allow_discount']) ?>>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="max_discount" id="max_discount" class="form-control" step="0.01" 
                                                       value="<?= editOld('max_discount', $product['max_discount']) ?>" placeholder="0.00">
                                                <span class="input-group-text">%</span>
                                            </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="max_stock" class="form-control" 
                                                   value="<?= editOld('max_stock', $product['max_stock']) ?>" placeholder="0">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="min_stock" class="form-control" 
                                                   value="<?= editOld('min_stock', $product['min_stock']) ?>" placeholder="0">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="reorder_point" class="form-control" 
                                                   value="<?= editOld('reorder_point', $product['reorder_point']) ?>" placeholder="0">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                            ...
                                            <div class="location-row row mb-2">
                                                <div class="col-md-3">
                                                    <input type="text" name="locations[]" class="form-control" 
                                                           value="<?= htmlspecialchars($location['location_code']) ?>" placeholder="كود الموقع">
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="text" name="location_names[]" class="form-control" 
                                                           value="<?= htmlspecialchars($location['location_name']) ?>" placeholder="اسم الموقع">
                                                </div>
                                                <div class="col-md-2">
                                                    <input type="text" name="shelf_numbers[]" class="form-control" 
                                                           value="<?= htmlspecialchars($location['shelf_number'] ?? '') ?>" placeholder="رف">
                                                </div>
                                                <div class="col-md-2">
                                                    <input type="text" name="row_numbers[]" class="form-control" 
                                                           value="<?= htmlspecialchars($location['row_number'] ?? '') ?>" placeholder="صف">
                                                </div>
                                                <div class="col-md-2">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="HIGH_ALERT" class="form-check-input" id="alert_high" <?= in_array('HIGH_ALERT', $existing_alerts) ? 'checked' : '' ?>>
                                        ...
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="TOXIC" class="form-check-input" id="alert_toxic" <?= in_array('TOXIC', $existing_alerts) ? 'checked' : '' ?>>
                                        ...
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="SOUNDALIKE" class="form-check-input" id="alert_sound" <?= in_array('SOUNDALIKE', $existing_alerts) ? 'checked' : '' ?>>
                                        ...
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="LOOKALIKE" class="form-check-input" id="alert_look" <?= in_array('LOOKALIKE', $existing_alerts) ? 'checked' : '' ?>>
                                    ...
                                    <div class="col-md-6">
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="CONTRAINDICATION" class="form-check-input" id="alert_contra" <?= in_array('CONTRAINDICATION', $existing_alerts) ? 'checked' : '' ?>>
                                        ...
                                        <div class="alert-select">
                                            <div class="form-check">
                                                <input type="checkbox" name="alerts[]" value="PREGNANCY" class="form-check-input" id="alert_preg" <?= in_array('PREGNANCY', $existing_alerts) ? 'checked' : '' ?>>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkDuplicate(field, value, feedbackId) {
            ...
            validationTimeout[field] = setTimeout(() => {
                fetch(`../../api/check-product.php?field=${field}&value=${encodeURIComponent(value)}&exclude_id=${productId}`)
